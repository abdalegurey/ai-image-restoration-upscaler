...
type RestoreResponse = {
  allupscaler: UpsclaerItem[];
};
const page = () => {

     const [data, setData] = useState<UpsclaerItem[]>([]);
      const [loading, setLoading] = useState(true);
    
     useEffect(() => {
      const fetchdata = async () => {
        try {
          const restore = await fetch("/api/readaupscaler");
        ...
        const cleaned = res.allupscaler.map((item: UpsclaerItem) => ({
              ...item,
      ...
      >
        ...
        <div className="mt-4 flex gap-2">
          <Button
            ...
            size="sm"
            className="flex-1"
            onClick={() => copyImage(item.upscalerImage)}
          >
          ...
          <Button
            ...
            onClick={() =>
              downloadImage(item.upscalerImage, `restored-${item.id}.png`)
            }
          >
